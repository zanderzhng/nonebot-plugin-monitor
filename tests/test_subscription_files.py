...
def test_subscription_structure():
    ...
    required_files = ["__init__.py", "template.py"]
...
def test_site_files_content():
    ...
    # Test __init__.py has functional approach definitions
    init_content = (sites_path / "__init__.py").read_text(encoding="utf-8")
    assert "SiteConfig" in init_content
    assert "FetchFunc" in init_content
    assert "CompareFunc" in init_content
    assert "FormatFunc" in init_content
    assert "ScheduleFunc" in init_content
    assert "DescriptionFunc" in init_content

    # Test template has expected structure for functional approach
    template_content = (sites_path / "template.py").read_text(encoding="utf-8")
    assert "site =" in template_content
    assert "SiteConfig" in template_content
    ...
    assert "site =" in example_content
    assert "SiteConfig" in example_content
